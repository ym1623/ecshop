...
$cache_id = sprintf('%X', crc32($cache_id));
    ...
    /* 如果没有缓存，生成缓存 */
if (!$smarty->is_cached('group_seconds.dwt', $cache_id))
{
	    $count = group_buy_count($city_id,$cat_id,2);
        
		if ($count > 0)
        {
            /* 取得当前页的秒杀商品*/
            $group_list = group_buy_list($city_id,$cat_id,2,$size, $page);
            $smarty->assign('group_list',  $group_list);
            /* 设置分页链接 */
            $pager = get_group_pager('seconds.php', array('catid'=>$cat_id,'cityid' => $city_id), $count, $page, $size);
	    ...
	    if (in_array(2,$show_groupclass))
	    {
		  $smarty->assign('catid',  $cat_id);
		  $smarty->assign('class_url',  rewrite_groupurl('seconds.php'));
		  $smarty->assign('class_list', group_class_list(1,true,'seconds.php'));
		}
		$smarty->assign('where', 'seconds');
		//$smarty->assign('today_group',  get_today_grouplist('0',$city_id, $cat_id));

}
$smarty->display('group_seconds.dwt', $cache_id);
