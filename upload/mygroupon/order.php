...

{
...

    {
...

        {
            ...
            $integral = integral_group_give($order['extension_id']);
...
elseif ($_REQUEST['act'] == 'edit_postscript')
{
    //check_authz_json('goods_manage');
    $order_id = intval($_POST['id']);
    $postscript = json_str_iconv(trim($_POST['val']));
    $sql = "SELECT shipping_status FROM " . $GLOBALS['ecs']->table('order_info') . " WHERE order_id = '$order_id'";
	if ($db->getOne($sql) == 0)
	{

      $sql = "UPDATE " . $ecs->table('order_info') . " SET postscript='$postscript' WHERE order_id='$order_id'";
     if ($db->query($sql))
      {
        clear_cache_files();
        make_json_result(stripslashes($postscript));
     }
	}
}
...

{
...

    {
        ...
        $sql = "SELECT o.order_id, o.order_sn, o.add_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid," .
                    ...
                    "o.pay_status, o.consignee, o.address, o.email, o.tel, o.extension_code, o.extension_id,o.pay_name,o.postscript, " .
...
function get_today_ordernum()
{
	...
	$tsql = " AND o.add_time >= '$start_time' AND o.add_time <= '$end_time'";
	$osql = " AND o.add_time >= '$ostart_time' AND o.add_time <= '$oend_time'";
	$sql = "SELECT COUNT(*) AS total_order " .
            ...
            " AND o.pay_status " . db_create_in(array(PS_PAYED, PS_PAYING));
    $tpaynum = $GLOBALS['db']->getOne($sql.$tsql);
	$zpaynum = $GLOBALS['db']->getOne($sql.$osql);
	$sql = "SELECT COUNT(*) AS total_order " .
            "FROM " . $GLOBALS['ecs']->table('order_info') . " AS o" .
            " WHERE o.extension_code = 'group_buy'";
			//" AND o.order_status " . db_create_in(array(OS_CONFIRMED, OS_SPLITED, OS_SPLITING_PART));
    $tordernum = $GLOBALS['db']->getOne($sql.$tsql);
	$zordernum = $GLOBALS['db']->getOne($sql.$osql);
   return array('paynum'=>$tpaynum,'ordernum'=>$tordernum,'zpaynum'=>$zpaynum,'zordernum'=>$zordernum);
}

function integral_group_give($group_id)
{
   //$group_id = intval($order['extension_id']); 
   $sql = "SELECT group_id,ext_info FROM " . $GLOBALS['ecs']->table('group_activity') .
              " WHERE group_id = '$group_id'";
   $group_buy = $GLOBALS['db']->getRow($sql);
   $ext_info = unserialize($group_buy['ext_info']);
   $integral = $ext_info['gift_integral'];
   return $integral;
}
