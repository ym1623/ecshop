...
elseif ($_REQUEST['act'] == 'main_api')
{
    ...
    ' FROM ' .$ecs->table('order_info'));
        ...
        $group_shopname = $db->getOne('SELECT value FROM ' . $ecs->table('shop_config') . ' WHERE code = \'group_shopname\'');
        /*if($style == '')
        {
            $style = '0';
        }*/
        ...
        $apiget = "ver= $ecs_version &lang= $ecs_lang &release= $ecs_release &php_ver= $php_ver &mysql_ver= $mysql_ver &ocount= $ocount &oamount= $oamount &gcount= $gcount &charset= $ecs_charset &usecount= $ecs_user &group_shopname= $group_shopname&url= $shop_url &patch= $patch_file ";

        $t = new transport;
        $api_comment = $t->request('http://www.ecgroupon.com/checkver.php', $apiget);
