...
elseif ($_REQUEST['act'] == 'addtolist')
{
    ...
    if (!empty($template_id))
    {
        ...
        {
			$countsql = "SELECT COUNT(*) FROM " . $ecs->table('email_list') . " WHERE stat = 1";
			if ($city_id > 0)
			{
			   $countsql .= " AND city_id='$city_id'";	
			}
            $count = $db->getOne($countsql);
            if ($count > $start)
            {
				$citysql = $city_id > 0 ? " AND city_id='$city_id'" : '';
                $sql = "SELECT email FROM " . $ecs->table('email_list') . " WHERE stat = 1 $citysql LIMIT $start,100";
                $query = $db->query($sql);
                $add = '';
        ...
        else
        {
            ...
            {
                $count_sql = 'SELECT COUNT(*) FROM ' . $ecs->table('users') . " WHERE is_validated = 1 $citysql";
                $email_sql = 'SELECT email FROM ' . $ecs->table('users') . " WHERE is_validated = 1 $citysql LIMIT $start,100";
            }
            elseif ($row['special_rank'])
            {
                $count_sql = 'SELECT COUNT(*) FROM ' . $ecs->table('users') . " WHERE is_validated = 1 $citysql AND user_rank = " . $rank_array['1'];
                $email_sql = 'SELECT email FROM ' . $ecs->table('users') . " WHERE is_validated = 1 $citysql AND user_rank = " . $rank_array['1'] . " LIMIT $start,100";
            }
            else
            {
				$citysql = $city_id > 0 ? " AND u.city_id='$city_id'" : '';
                $count_sql = 'SELECT COUNT(*) ' .
                             ...
                             " WHERE ur.rank_id = '" . $rank_array['1'] . "' AND u.is_validated = 1 $citysql";
                $email_sql = 'SELECT u.email ' .
                             'FROM ' . $ecs->table('users') . ' AS u LEFT JOIN ' . $ecs->table('user_rank') . ' AS ur ' .
                             "  ON ur.special_rank = '0' AND ur.min_points <= u.rank_points AND ur.max_points > u.rank_points" .
                             " WHERE ur.rank_id = '" . $rank_array['1'] . "' AND u.is_validated = 1 $citysql LIMIT $start,100";
            }
