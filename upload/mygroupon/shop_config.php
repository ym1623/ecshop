...
if ($_REQUEST['act'] == 'list_edit')
{
	...
	{
	  $_CFG['showclass'][1] = true;
	}
	if (in_array(2,$show_groupclass))
	{
	  $_CFG['showclass'][2] = true;
	}
	if (in_array(3,$show_groupclass))
	{
	  $_CFG['showclass'][3] = true;
	}
...
elseif ($_REQUEST['act'] == 'post')
{
	...
	$show_groupclass = !empty($_POST['show_groupclass']) ? implode(',',$_POST['show_groupclass']) : '';
    $sql = "UPDATE " . $ecs->table('shop_config') . " SET value = '$show_groupclass' WHERE code = 'show_groupclass'";
    $db->query($sql);
    /* 记录日志 */
